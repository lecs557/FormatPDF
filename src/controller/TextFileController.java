		...
		if (!analizeOnly){
			...
			if(counter>3){
				...
				for (int i = 0; i < length; i++) {
					if (i==0){
						fw.append("[{product_id:2,number:"+counter+",title:"+today.getParagraphs().get(0).getParagraph()+",\r\n");
					}else
						fw.append("text:<p style=\"margin-left:0cm; margin-right:0cm\">"+today.getParagraphs().get(i).getParagraph()+"</p>\r\n");
					if(i==length-1)
				...
				for(Chapter ch:chapter){
					if(a!=0){
						for (int i=0; i<ch.getParagraphs().size();i++){	
							if (i==0){
								fw.append("[{product_id:2,number:1,title:"+ch.getParagraphs().get(0).getParagraph()+",\r\n");
							}else
								fw.append("text:<p style=\"margin-left:0cm; margin-right:0cm\">"+ch.getParagraphs().get(i).getParagraph()+"</p>\r\n");
